    ...
    if plot == 2:
        ...
        if normalize == 1:
            ...
            if setptMethod == "automatic" and objType == "flow":
                ...
                plt.plot(time,setpt_WRRF_flow*np.ones(len(WRRF_flow)), color = 'k', label = 'Setpoint')
        else:
            ...
            if setptMethod == "automatic" and objType == "flow":
                ...
                plt.plot(time,max_flow_WRRF*setpt_WRRF_flow*np.ones(len(WRRF_flow)), color = 'k', label = 'Setpoint')
    elif plot == 4:
        if normalize == 1:
            ...
            if setptMethod == "automatic" and objType == "TSS":
                ...
                plt.plot(time,setpt_WRRF_TSS*np.ones(len(WRRF_TSSLoad)), color = 'k', label = 'Setpoint')
        else:
            plt.plot(time,WRRF_TSSLoad, label = "MBC, WRRF TSS Load", color = colors[-1],
                linestyle = '-')
            if setptMethod == "automatic" and objType == "TSS":
                plt.plot(time,max_TSSLoad_WRRF*setpt_WRRF*np.ones(len(WRRF_TSSLoad)), color = 'k', label = 'Setpoint')
            elif setptMethod == "automatic" and objType == "both":
                plt.plot(time,max_TSSLoad_WRRF*setpt_WRRF_TSS*np.ones(len(WRRF_TSSLoad)), color = 'k', label = 'Setpoint')
    elif plot == 5:
        for a in range(0,n_trunkline):
            if normalize == 1:
                plt.plot(time,dstream_flows[:,a]/max_flow_dstream[a],
