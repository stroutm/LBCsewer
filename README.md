...
|-----------|:-----------:|-----------|
| Set of sewer assets | ![i\in I](https://latex.codecogs.com/gif.latex?i\in&space;I) | These can include e.g. storage basins with outlet valves and pump stations. |
| States | ![S_{i}](https://latex.codecogs.com/gif.latex?S_{i}) | This can be a vector of multiple states for each asset ![i](https://latex.codecogs.com/gif.latex?i) and can include items such as water level, pollutant concentration, and flow. |
| Setpoints | ![S_{i}^*](https://latex.codecogs.com/gif.latex?S_{i}^*) | Setpoints are desired system conditions and must be specified for each state in ![S_{i}](https://latex.codecogs.com/gif.latex?S_{i}). |
| System importance values | ![\alpha_{i}](https://latex.codecogs.com/gif.latex?\alpha_{i}) | This can be used to specify which assets and states are most important within the system. The size of ![\alpha_{i}](https://latex.codecogs.com/gif.latex?\alpha_{i}) must match ![S_{i}](https://latex.codecogs.com/gif.latex?S_{i}). |
| Instantaneous importance weight | ![\rho](https://latex.codecogs.com/gif.latex?\rho) | This parameter can be tuned to reflect how stressed an asset is at any given point in time. By analogy, ![\rho](https://latex.codecogs.com/gif.latex?\rho) could encapsulate the comfort level of an operator (e.g., release proportional to water level vs. prioritize an asset only if it is close to full). For example, if storage capacity is used as an indicator of importance, with ![\rho=1](https://latex.codecogs.com/gif.latex?\rho=1) the instantaneous importance of the asset increases nearly linearly with water level. With ![\rho=100](https://latex.codecogs.com/gif.latex?\rho=100) the asset would be considered important only if it is close to capacity. See the graph in the above figure for further illustration of how ![\gamma_{i}](https://latex.codecogs.com/gif.latex?\gamma_{i}) is impacted by state ![S_{i}](https://latex.codecogs.com/gif.latex?S_{i}) for different values of ![\rho](https://latex.codecogs.com/gif.latex?\rho). |
| Set of controllable assets | ![I_{C}](https://latex.codecogs.com/gif.latex?I_{C}) | This set should contain all sewer storage assets in ![I](https://latex.codecogs.com/gif.latex?I) that are controllable by the operator or system. This algorithm will inform control actions for these assets. |
| Set of uncontrollable assets | ![I_{U}](https://latex.codecogs.com/gif.latex?I_{U}) | This set should contain all sewer storage assets in ![I](https://latex.codecogs.com/gif.latex?I) that will remain uncontrolled or are passively controlled. An example of what this set might contain is the inlet at the downstream WRRF with states pertaining to inflow conditions. Note: ![I_{C}\cup I_{U}=I](https://latex.codecogs.com/gif.latex?I_{C}\cup&space;I_{U}=I) |
...
| Set of assets to release water | ![J](https://latex.codecogs.com/gif.latex?J) | These assets must be controllable (![J\subseteq I_{C}](https://latex.codecogs.com/gif.latex?J\subseteq&space;I_{C})) and have an importance-weighted deviation that is greater than the average: asset ![j](https://latex.codecogs.com/gif.latex?j) such that ![\beta_j(t)\cdot\left(S_j(t)-S_j^*(t)\right)>\bar{C}(t)](https://latex.codecogs.com/gif.latex?\beta_j(t)\cdot\left(S_j(t)-S_j^*(t)\right)>\bar{C}(t)). |
| Relative allotment factors | ![R_{j}](https://latex.codecogs.com/gif.latex?R_{j}) | This factor is computed for each asset that will release water, i.e. ![j\in J](https://latex.codecogs.com/gif.latex?j\in&space;J). This allotment factor simply assigns the fraction of a downstream asset’s capacity that will be allotted to an upstream asset ![j](https://latex.codecogs.com/gif.latex?j) and is defined as the importance-weighted deviation that is then normalized within set ![J](https://latex.codecogs.com/gif.latex?J). |
